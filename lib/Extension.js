!function() {
    ...
    module.exports = new Class({
                ...
                this.orm[model.getDefinition().getDatabaseName()][model.getEntityName()](model.getPrimaryKeyFilter(), ['*']).findOne(function(err, dbModel) {
                    if (err) callback(err);
                        ...
                        if (groupKey && model[groupKey] != dbModel[groupKey]) {
                            ...
                            // will not move subtrees ... thats too complicated for the moment!
                            if (dbModel[this.right]-dbModel[this.left] !== 1) callback(new Error('Cannot move subtree to another group, please move the nodes induvidually!'));
                            else {
                                // emulate an insert
                                this.onBeforeInsert(model, transaction, function(err) {
                                    if (err) callback(err);
                                    else {

                                        // emulate delete
                                        this.onAfterDelete(dbModel, transaction, callback);
                                    }
                                }.bind(this));
                            }
                        }
         ...
         * tree
         */
        , syncTree: function(tree, callback) {
         ...
         */
        , loadTree: function(groupKey, callback) {
        ...
        , _executeQuery: function(model, filters, values, transaction, callback) {
            ...
            this.orm[databaseName][entityName](this._createfilter(null, definition, databaseName, entityName, filters, model)).update(this._createObject(values), callback, transaction);
        }
