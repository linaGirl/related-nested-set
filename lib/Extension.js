!function(){
    ...
    module.exports = new Class({
        ...
        , onBeforeInsert: function(model, transaction, callback) {
                ...
                this._getTargetModelInstance(model, transaction, function(err, node) {
                    if (err) callback(err);
                        ...
                        this._executeQuery(model, [{
                              key       : this.left
                            , value     : this.orm.getORM().gte(model[this.left])
                        }], [{
                              key       : this.left
                            , value     : this.orm.getORM().increaseBy(2)
                        }], transaction, wait());

                        this._executeQuery(model, [{
                              key       : this.right
                            , value     : this.orm.getORM().gte(model[this.right])
                        }], [{
                              key       : this.right
                            , value     : this.orm.getORM().increaseBy(2)
                        }], transaction, wait());
            ...
            else callback(new Error('No node position defined for the nested set on the model «'+model.getEntityName()+'». Please define one via the setParent, append or the prepend method.'));
        }
        ...
        , _getTargetModelInstance: function(model, transaction, callback) {
            ...
            else if (model._nestedSet.append) input = model._nestedSet.append;
            else if (model._nestedSet.prepend) input = model._nestedSet.prepend;
