!function(){
    ...
    module.exports = new Class({
            ...
            if (this._nestedSet) {
                ...
                this._getTargetModelInstance(model, transaction, function(err, node) {
                    if (err) callback(err);
                        ...
                        this._executeQuery(model, [{
                              key       : this.right
                            , value     : ORM.gte(model[this.right])
                        }], [{
                              key       : this.right
                            , value     : ORM.increaseBy(2)
                        }], transaction, wait());
        ...
        /*
         * prepend this node to the targetnode
         * runs in the context of the model
         */
        , prepend: function(targetNode) {
            thisContext._prepareForNestedSet(this);
            this._nestedSet.prepend = targetNode;
        }

        /*
         * append this node after the targetnode
         * runs in the context of the model
         */
        , append: function(targetNode) {
            thisContext._prepareForNestedSet(this);
            this._nestedSet.append = targetNode;
        }
        ...
        , _getTargetModelInstance: function(model, transaction, callback) {
            ...
            if (this._nestedSet.parent) input = this._nestedSet.parent;
            else if (this._nestedSet.append) input = this._nestedSet.append;
            else if (this._nestedSet.prepend) input = this._nestedSet.prepend;

            if (this._nestedSet.newRootNode && this._nestedSet.asLastChild) {
                // try to get the last node
                definition  = odel.getDefinition();
                filter      = this._createfilter(null, definition, databaseName, entityName);

                // go and find the last node
                this.orm[databaseName][entityName](filter, ['*']).order(this.right, true).findOne(transaction, callback);
            }
                ...
                if (input.isModel()) model.reload(transaction, callback);
                else {
                    input.findOne(transaction, function(err, node) {
                        if (err) callback(err);
                ...
                this.orm[databaseName][entityName](filter, ['*']).findOne(transaction, function(err, node) {
                    if (err) callback(err);
        ...
        , _executeQuery: function(model, filters, values, transaction, callback) {
            ...
            this._orm[databaseName][entityName](this._createfilter(null, definition, databaseName, entityName, filters)).update(this._createObject(values), callback, transaction);
        }
        ...
        , _prepareForNestedSet: function(target) {
            if (!target._neestedSet) Class.define(target, '_nestedSet', Class({}).Writable().Configturable());
        }
        ...
        , applyModelMethods: function(definition, classDefinition) {
            if (definition.hasColumn(this.deleted)) {
                ...
                classDefinition.append      = this.append;
                classDefinition.prepend     = this.prepend;
            }
        }
