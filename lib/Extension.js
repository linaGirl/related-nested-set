    ...
    module.exports = new Class({
        ...
        , onBeforeInsert: function(model, transaction, callback, width) {
            ...
            if (model._nestedSet) {
                ...
                if (model.nestedSetContext.hasGroupKey(model)) {
                    groupKeyName = model.nestedSetContext.getGroupKey(model);
         ...
         * tree
         */
        , syncTree: asyncMethod(function(tree, callback, context) {
            ...
            if (context.hasGroupKey(this)) {
                groupKeyName = context.getGroupKey(this);


                groupKey = context.getGroupKeyFromTree(groupKeyName, tree);
            ...
            filter = context._createfilter(null, definition, definition.getDatabaseName(), definition.getTableName(), null, pseudoModel);
         ...
         */
        , loadTree: asyncMethod(function(groupKey, callback, context) {
            ...
            filter = context._createfilter(null, definition, databaseName, entityName, null, pseudoModel);
            ...
            this.find(function(err, set) {
                if (err) callback(err);
                    ...
                    // build tree
                    context._parseNodes(virtualParent, set);
         ...
         * runs in the context of the model
         */
        , setParent: function(node, asLastChild, context) {
            context._prepareForNestedSet(this);
         ...
         * runs in the context of the model
         */
        , before: function(targetNode, context) {
            context._prepareForNestedSet(this);
         ...
         * runs in the context of the model
         */
        , after: function(targetNode, context) {
            context._prepareForNestedSet(this);
         ...
         */
        , _repositionModel: function(model, transaction, callback, context) {
            if (model.disableNestedSetExtension) return callback();
                        ...
                        if (err) callback(err);
                            ...
                            if (nestedSet.newRootNode) {
                                ...
                                if (!node) return callback(new Error('Failed to load the target node for repositioning the model!'));
                                    ...
                                    if (context.hasGroupKey(model)) {
                                        groupKeyName = context.getGroupKey(model);
                                        if (!model[groupKeyName]) return callback(new Error('Cannot save model, missing grouping key for the nesting set «'+model.getEntityName()+'»!'));
        ...
        , applyModelMethods: function(definition, classDefinition) {
            ...
            classDefinition.setParent       = this.proxyCall(this.setParent);
            classDefinition.after           = this.proxyCall(this.after);
            classDefinition.before          = this.proxyCall(this.before);
            classDefinition.getChildren     = this.proxyCall(this.getChildren);
            classDefinition.getParentNode   = this.proxyCall(this.getParentNode);

            classDefinition.nestedSetContext = this;
        ...
        , applyQueryBuilderMethods: function(definition, classDefinition) {
            ...
            classDefinition.loadTree = this.proxyCall(this.loadTree);
            classDefinition.syncTree = this.proxyCall(this.syncTree);
        }
        ...
        /**
        * proxy calls from the orm to this class due to horrile design mistakes :(
        */
        , proxyCall(method) {
            const localContext = this;

            return function(...params) {
                return method.call(this, ...params, localContext);
            };
        }
